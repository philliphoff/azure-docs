...
## Optimizing Durable Task Scheduler workers

The Durable Task Scheduler breaks down orchestration execution into a series of orchestration, activity, and entity "work items". Work items are then distributed across connected workers and their results persisted by the scheduler. These persisted results are what allows execution to resume when encountering transient failures.

Each worker connected to the Durable Task Scheduler specifies the maximum number of work items for orchestration, activity, and entity execution it is willing to accept at any given time. These values are generally configurable through the Durable Task SDK and typically default to some multiple of the number of CPUs on which the worker is running. The Durable Task Scheduler uses those values to determine which worker receives new work (and when).

Optimizing the Durable Task Scheduler workers means matching the number and compute capacity of workers to the expected creation rate on the scheduler and the work required of each scheduled orchestration. Having too many workers or too much compute capacity may leave the scheduler mostly idle, which may be a waste of resources. Having too few workers or too little compute capacity may oversaturate workers and cause orchestrations to remain pending, waiting for available compute capacity, resulting in longer-running orchestrations.

It is also possible for workers to be "right-sized" but not optimally configured--their maximum work item limits not appropriately set for the type of work being performed.  Workers might be able to theoretically do more work but are artificially limited because low work item limits prevent the scheduler from distributing additional work. For CPU-intensive work, workers might become oversaturated because high work item limits cause the scheduler to give it more work than it can handle. When testing application performance, it is important to consider worker configuration in addition to horizonal and vertical scale.
